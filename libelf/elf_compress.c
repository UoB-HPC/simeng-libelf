...
static void *do_deflate_cleanup(void *result, z_stream *z, void *out_buf,
                                Elf_Data *cdatap) {
  deflateEnd(z);
  free(out_buf);
  if (cdatap != NULL)
...
static void *__libelf_compress_zlib(Elf_Scn *scn, size_t hsize, int ei_data,
...
/* Cleanup and return result.  Don't leak memory.  */
static void *do_zstd_cleanup(void *result, ZSTD_CCtx *const cctx, void *out_buf,
                             Elf_Data *cdatap) {
  ZSTD_freeCCtx(cctx);
  free(out_buf);
  if (cdatap != NULL)
...
static void *__libelf_compress_zstd(Elf_Scn *scn, size_t hsize, int ei_data,
                                    size_t *orig_size, size_t *orig_addralign,
                                    size_t *new_size, bool force,
                                    Elf_Data *data, Elf_Data *next_data,
                                    void *out_buf, size_t out_size,
...
void *internal_function __libelf_compress(Elf_Scn *scn, size_t hsize,
                                          int ei_data, size_t *orig_size,
                                          size_t *orig_addralign,
                                          size_t *new_size, bool force,
                                          bool use_zstd) {
  ...
  if (next_data == NULL && !force && data->d_size <= hsize + 5 + 6)
    return (void *)-1;
  ...
  *orig_size = data->d_size;
  ...
  if (use_zstd) {
    ...
    return __libelf_compress_zstd(scn, hsize, ei_data, orig_size,
                                  orig_addralign, new_size, force, data,
                                  next_data, out_buf, out_size, block);
#else
...
void *internal_function __libelf_decompress_zlib(void *buf_in, size_t size_in,
                                                 size_t size_out) {
...
#ifdef USE_ZSTD
static void *__libelf_decompress_zstd(void *buf_in, size_t size_in,
                                      size_t size_out) {
  /* Malloc might return NULL when requesting zero size.  This is highly
     unlikely, it would only happen when the compression was forced.
     But we do need a non-NULL buffer to return and set as result.
...
void *internal_function __libelf_decompress(int chtype, void *buf_in,
...
void *internal_function __libelf_decompress_elf(Elf_Scn *scn, size_t *size_out,
                                                size_t *addralign) {
  GElf_Chdr chdr;
  ...
  size_t hsize =
      (elfclass == ELFCLASS32 ? sizeof(Elf32_Chdr) : sizeof(Elf64_Chdr));
...
void internal_function __libelf_reset_rawdata(Elf_Scn *scn, void *buf,
  ...
  if (scn->data_base != scn->rawdata_base)
    free(scn->data_base);
  scn->data_base = NULL;
  if (scn->zdata_base != buf && scn->zdata_base != scn->rawdata_base) {
    free(scn->zdata_base);
    scn->zdata_base = NULL;
  }
  if (scn->elf->map_address == NULL || scn->rawdata_base == scn->zdata_base ||
...
int elf_compress(Elf_Scn *scn, int type, unsigned int flags) {
  if (scn == NULL)
    return -1;
  ...
  if (type == ELFCOMPRESS_ZLIB || type == ELFCOMPRESS_ZSTD) {
    ...
    void *out_buf =
        __libelf_compress(scn, hsize, elfdata, &orig_size, &orig_addralign,
                          &new_size, force, type == ELFCOMPRESS_ZSTD);
    ...
    __libelf_reset_rawdata(
        scn, scn->zdata_base, scn->zdata_size, scn->zdata_align,
        __libelf_data_type(&ehdr, sh_type, scn->zdata_align));
